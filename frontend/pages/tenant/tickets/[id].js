  ...
  return (
    <Layout title={`Ticket ${ticket.ticket_number}`}>
      <div className="max-w-3xl mx-auto space-y-5">
        <Link href="/tenant/tickets" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm text-gray-600">
            ...
            {ticket.staff_name && (
              <div className="flex items-center gap-2">
                ...
                Assigned to: <strong>{ticket.staff_name}</strong>
              </div>
            )}
            {ticket.sla_deadline && (
                ...
                <div className="flex-1 bg-gray-50 rounded-xl p-4 border border-gray-100">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded capitalize font-medium ${ROLE_COLOR[u.updater_role] || 'bg-gray-100 text-gray-600'}`}>
                        {u.updater_role}
                      </span>
                    </div>
                        ...
                        {isImg ? (
                          <a href={`${API_BASE}/uploads/${u.attachment}`} target="_blank" rel="noopener noreferrer">
                            <img src={`${API_BASE}/uploads/${u.attachment}`} alt="Attachment"
                              className="max-w-xs max-h-48 rounded-lg border object-cover hover:opacity-90 cursor-pointer" />
                          </a>
                        ) : (
            ...
            <form onSubmit={handleAddUpdate} className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button type="submit" disabled={posting}
                  className="ml-auto flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-60 transition-colors">
