...
export default function SecurityTicketDetail() {
  ...
  return (
    <Layout title={`Incident ${ticket.ticket_number}`}>
      <div className="max-w-3xl mx-auto space-y-5">
        <Link href="/security/tickets" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${PRIORITY_COLOR[ticket.priority]}`}>{ticket.priority}</span>
              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${STATUS_COLOR[ticket.status]}`}>{ticket.status}</span>
            </div>
          </div>
          {ticket.description && (
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              ...
              Logged on {fmt(ticket.created_at)}
            </div>
            {ticket.category && (
              <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            {ticket.updates?.map((u) => (
              <div key={u.id} className="flex gap-3">
                <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold ${ROLE_COLOR[u.updater_role] || 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <div className="mt-3">
                        {isImg ? (
                          <a href={`${API_BASE}/uploads/${u.attachment}`} target="_blank" rel="noopener noreferrer">
                            <img src={`${API_BASE}/uploads/${u.attachment}`} alt="Evidence"
                          ...
                          <a href={`${API_BASE}/uploads/${u.attachment}`} target="_blank" rel="noopener noreferrer" download
                            ...
                            <PaperClipIcon className="w-3.5 h-3.5" /> Download File
                          </a>
                        )}
                      </div>
                    );
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
            ...
            <form onSubmit={handleAddUpdate} className="space-y-3">
              <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
                ...
                placeholder="Additional details or follow-up..." />
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button type="submit" disabled={posting}
                  className="ml-auto flex items-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-medium px-4 py-2 rounded-lg disabled:opacity-60 transition-colors">
